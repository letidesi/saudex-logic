import { AdministratorsReadListUseCase, IAdministratorsReadListDTO, IAdministratorsReadListResponse } from "../../../modules/administrators";
...
function createSUT(): {
	...
	const administratorsReadListUseCase = new AdministratorsReadListUseCase(administratorsServiceMock);
...
describe('Given no parameters, with valid Administrators in the repository', () => {
    ...
    it('should list the first page of the Administrators', async () => {
        const AdministratorsReadListResponse: IAdministratorsReadListResponse = await sut.exec({});
	...
	describe('given email parameter', () => {
		...
		it('should return a list with the correct email', async () => {
			const correctAdministratorss = administratorsMockList.filter((administrators) => administrators.email === email);
