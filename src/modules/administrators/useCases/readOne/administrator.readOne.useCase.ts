...
import { IAdministratorReadOneDTO, IAdministratorReadOneResponse } from "./administrator.readOne.DTO";
...
export class AdministratorReadOneUseCase {
	 ...
	 */
	async exec(dto: IAdministratorReadOneDTO): Promise<IAdministratorReadOneResponse> {
		...
		if (email) administrator = await this.administratorRepository.readOneByEmail({ email });
		if (administratorId) administrator = await this.administratorRepository.readOne({ administratorId });
