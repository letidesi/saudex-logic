...
import { IAdministratorsReadListParamsPagination, IAdministratorsRepository } from '../../repository/administrators.repository';
import { IAdministratorsReadListDTO, IAdministratorsReadListResponse } from './administratorsReadList.DTO';
...
export class AdministratorsReadListUseCase {
	 ...
	 */
	async exec(dto: IAdministratorsReadListDTO): Promise<IAdministratorsReadListResponse> {
		...
		const list: Array<IAdministrators> = await this.administratorsRepository.readList(parameters);
		const count: number = await this.administratorsRepository.readListCount(parameters);
